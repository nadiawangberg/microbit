...
uint32_t bluetooth_gap_advertise_start(){
	...
	// Definerer melding som skal sendes
	static uint8_t adv_data[] = { // This is one / more AD struct, which is <= 31byte
		8, BLE_GAP_AD_TYPE_COMPLETE_LOCAL_NAME,
		'h', 'e', 'i', ' ', 'x', 'D', '!',
		3, BLE_GAP_AD_TYPE_16BIT_SERVICE_UUID_COMPLETE,
		0x0D, 0xF0
	};
	uint8_t adv_data_length = 13; // Må ikke gå over 31
	err_code = sd_ble_gap_adv_data_set(adv_data, adv_data_length, NULL, 0); // setter inn dataen (NULL, ingen scan response pkg)

	//Starter sendingen
	ble_gap_adv_params_t adv_params; //Definerer parameterstruktur, som senere enabler sending / gjør sending
	memset(&adv_params, 0, sizeof(adv_params)); //Nuller ut uinterressant felter i strukten

	adv_params.type = BLE_GAP_ADV_TYPE_ADV_IND;// setter type (velger måte å kringkaste på)
	adv_params.interval = 0x0100; //setter feltet interval (100ms ishh?? hvor ofte meldinger blir sendt)
	sd_ble_gap_adv_start(&adv_params); //sender melding
	...
	return err_code; // 0 hvis ingen error
}
